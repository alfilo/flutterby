  ...
  Papa.parse("plants.csv", {
    ...
    complete: function (results) {
      ...
      if (location.search) {
        ...
        $(
          "#filter-group div:last-of-type .dropdown-content button:first-child"
        ).click();
...
$(function () {
  ...
  for (var i = 0; i < placeholders.length; i++) {
    ...
    if (
      i == 0 &&
      (location.pathname.includes("plants.html") ||
  ...
  try {
    ...
    if (location.pathname.includes("plants.html")) {
      $("#select-btns").before(
        "<p>Selections are also used to prepopulate Request" +
    ...
    if (location.pathname.includes("request.html")) {
      ...
      if (window.localStorage.length) {
        $("#plants").val(reqStr);
        $("#plants").before(
          "<p>The request below includes your selections." +
